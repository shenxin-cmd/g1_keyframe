...
CMD="cd '$REPO' && \
  ...
  python g1_batch_runner.py --resume --force --no-filter $FAST_IK_FLAG --workers $WORKERS --layers 1 --frames-per-ring 300 --fps 30 --seed-count 100 --batch-root '$BATCH_ROOT' 2>&1 | tee -a '$BATCH_ROOT/logs/run.log'; \
  echo ''; echo '[g1_batch] 批量任务已结束。窗口保持打开，exit 可关闭。'; \
  exec bash"

tmux new-session -d -s "$SESSION" bash -lc "$CMD"
echo "已启动 tmux 会话: $SESSION (workers=$WORKERS${G1_FAST_IK:+", fast-ik"})"
