"""
Parametric closed 2D paths in the YZ plane for G1 right-hand trajectory generation.
All shapes are generated in local (y, z) coordinates then mapped to world frame.
"""
...
def _to_world(local_yz: np.ndarray, center: np.ndarray, scale: float,
    ...
    """Map local (y,z) polyline to world (x,y,z). center = (cx,cy,cz)."""
...
def _resample_polyline(pts: list, N: int) -> np.ndarray:
    """Uniformly resample a closed polyline to N points."""
...
def _rounded_rect_local(half_y: float, half_z: float, N: int,
    ...
    # Clockwise in YZ (viewed from +x)
    seg(Ly - r, Lz, -Ly + r, Lz, N_seg)           # top
    _arc_corner_local(pts, -Ly + r, Lz - r, r, np.pi / 2, N_arc)
    seg(-Ly, Lz - r, -Ly, -Lz + r, N_seg)         # left
    _arc_corner_local(pts, -Ly + r, -Lz + r, r, 0.0, N_arc)
    seg(-Ly + r, -Lz, Ly - r, -Lz, N_seg)         # bottom
    _arc_corner_local(pts, Ly - r, -Lz + r, r, -np.pi / 2, N_arc)
    seg(Ly, -Lz + r, Ly, Lz - r, N_seg)           # right
...
def make_shape_waypoints(
    ...
    **kwargs,
    ...
    """
    Build N world-frame waypoints for a closed shape.

    Args:
        shape_name: one of SHAPE_NAMES
        center: (3,) cx, cy, cz
        scale: size multiplier
        theta: rotation in YZ plane (rad)
        N: number of points
        rng: required for random_* shapes
    """
    if rng is None:
    ...
    if name == "circle":
        local = make_circle_local(N)
    elif name == "square":
        local = make_square_local(N)
    elif name == "rectangle":
        local = make_rectangle_local(N, aspect=kwargs.get("aspect", 1.4))
    elif name == "triangle":
        local = make_triangle_local(N)
    elif name == "pentagon":
        local = make_pentagon_local(N)
    elif name == "star":
        local = make_star_local(N, inner_ratio=kwargs.get("inner_ratio", 0.45))
    elif name == "heart":
        local = make_heart_local(N)
    elif name == "wave":
        local = make_wave_local(N, n_lobes=kwargs.get("n_lobes", 3.0))
    elif name == "diamond":
        local = make_diamond_local(N)
    elif name == "random_polygon":
        local = make_random_polygon_local(N, rng)
    elif name == "random_spline":
        local = make_random_spline_local(N, rng)
    else:
